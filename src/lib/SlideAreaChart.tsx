...
import {
  ...
  SlideAreaChartComponentProps,
  SlideAreaChartDefaultProps,
  SlideAreaChartProps,
} from './utils/types';
...
class SlideAreaChart extends Component<SlideAreaChartComponentProps, State> {
  ...
  calculateDataToArray = (): Array<[number, number]> => {
    ...
    return data.length > 1
      ...
      ? data.reduce((arr: Array<[number, number]>, obj) => {
          return arr.concat([
            [Number(xRangeCalculated[0]), obj.y],
            [Number(xRangeCalculated[1]), obj.y],
          ]);
        }, [])
      : [
          [Number(xRangeCalculated[0]), yRangeCalculated[0]],
          [Number(xRangeCalculated[1]), yRangeCalculated[0]],
        ];
  };
  ...
  animateChart = (value: number) => {
    ...
    const cursorMarkerHeight =
      cursorProps?.cursorMarkerHeight ?? defaultCursorProps.cursorMarkerHeight;
    const cursorMarkerWidth =
      cursorProps?.cursorMarkerWidth ?? defaultCursorProps.cursorMarkerWidth;
    ...
    const toolTipX = oldX != null ? (1 - value) * oldX + x * value : x;
    if (this.toolTip.current != null) {
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      ...
      if (animated) {
        Animated.spring(this.state.cursorY, {
          toValue: 1,
          friction: 7,
  ...
  render() {
    ...
    const startLine = animated ? this.calculateStartLine() : this.calculateLine();
    ...
    return (
      ...
      >
        ...
        />
        {!onPress && <Cursor ref={this.cursor} {...combinedCursorProps} />}
