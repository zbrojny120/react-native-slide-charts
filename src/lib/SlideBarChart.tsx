...
import {
  defaultBarChartFillGradient,
  defaultBarChartSelectedFillGradient,
} from './components/chartComponents/charts/utils/colors';
import { PanGestureHandler } from 'react-native-gesture-handler';
import {
  ExtendedAnimatedValue,
  SlideBarChartDefaultProps,
  SlideBarChartComponentProps,
  SlideBarChartProps,
} from './utils/types';
...
class SlideBarChart extends Component<SlideBarChartComponentProps, State> {
  ...
  scaleX: ScaleTime<number, number> | ScaleLinear<number, number> = scaleLinear()
    .domain([0, 1])
    .range([0, 1]);
  scaleY: ScaleLinear<number, number> = scaleLinear().domain([0, 1]).range([0, 1]);
  line: string | null = null;
  chartWidth =
    this.props.width - this.props.axisWidth - this.props.paddingLeft - this.props.paddingRight;
  mounted = false;
  ...
  previousYRange = this.props.yRange
    ...
    : [getDataMin(this.props.data), getDataMax(this.props.data)];
  yRange: [number, number] = [getDataMin(this.props.data), getDataMax(this.props.data)];
  ...
  getX(
    selectedBarNumber: number,
    numberOfBars: number,
    barWidthForToolTip: number,
    barSpacingForToolTip: number,
    axisWidth: number,
    paddingLeft: number
  ): number {
    return numberOfBars > 1
      ? selectedBarNumber * barWidthForToolTip +
          selectedBarNumber * barSpacingForToolTip +
          barWidthForToolTip / 2 +
          axisWidth +
          paddingLeft
  ...
  animateChart = (value: number) => {
    const { axisWidth, yRange, data, barWidth, barSpacing, toolTipProps, paddingLeft } = this.props;
    ...
    const barWidthForToolTip =
      barWidth ?? (this.chartWidth - (numberOfBars - 1) * barSpacing) / numberOfBars;
  ...
  setChartToZero = () => {
    ...
    if (this.toolTip.current != null) {
      ...
      this.selectedBarNumber =
        Math.floor(xInput / barWidthSize) > 0
          ? Math.floor(xInput / barWidthSize) < numberOfBars - 1
            ? Math.floor(xInput / barWidthSize)
            : numberOfBars - 1
          : 0;
      this.setState({
        selectedBarNumber: alwaysShowIndicator ? this.selectedBarNumber : undefined,
      });
  ...
  moveCursorBinaryCore(value: number, isTouch: boolean) {
    ...
    if (callbackWithX) {
      callbackWithX(this.scaleX.invert(x));
    }
    if (callbackWithY) {
      callbackWithY(this.scaleY.invert(y));
    }
  ...
  renderTouchable() {
    ...
    if (onPress) {
      return (
        ...
        >
          <View style={[{ backgroundColor: '#FFFFFF00' }, StyleSheet.absoluteFill]} />
        </TouchableWithoutFeedback>
      );
    }
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      ...
      if (animated) {
        Animated.timing(this.state.cursorY, {
          toValue: 1,
          duration: 250,
  ...
  shouldComponentUpdate(nextProps: SlideBarChartProps) {
    ...
    this.yRange = [getDataMin(nextProps.data || []), getDataMax(nextProps.data || [])];
    return true;
  }
  ...
  componentDidUpdate(prevProps: SlideBarChartComponentProps) {
    ...
    const previousYRangeCalculated = prevProps.yRange
      ? prevProps.yRange
      : [getDataMin(prevProps.data), getDataMax(prevProps.data)];
    ...
    if (
      ...
      if (animated) {
        ...
        if (prevProps.data.filter(dataObject => dataObject.y !== 0).length > 0) {
          ...
          Animated.timing(this.state.toolTipY, {
            toValue: 0,
            duration: 250,
            useNativeDriver: true,
          }).start(() => {
  ...
  render() {
    ...
    const xCalculatedScale = xScale ? xScale : isValidDate(data[0]?.x ?? 0) ? 'time' : 'linear';
    this.scaleX =
      xCalculatedScale === 'linear'
        ? scaleLinear()
    ...
    return (
      ...
      >
        ...
        />
        {!onPress && <ToolTip ref={this.toolTip} {...toolTipProps} />}
