...
class ToolTip extends Component<ToolTipComponentProps, State> {
  ...
  setNativeTriangleXPositionProps = (realPercentage: number) => {
    ...
    if (this.triangle.current != null) {
      this.triangle.current.setNativeProps({
        left: (width * 0.8 - borderRadius * 2) * realPercentage + borderRadius,
      });
    }
  };
  ...
  renderTextInputs = () => {
    ...
    for (let i = 0; i < this.numberOfLines; i++) {
      ...
      const baseHeight =
        textStyles[i] != null && textStyles[i].fontSize != null ? textStyles[i].fontSize : fontSize;
    ...
    return (
      <View
        ...
        style={[
          ...
          lockTriangleCenter ? centerPosition : null,
        ]}
      >
  ...
  render() {
    ...
    if (!displayToolTip || this.textInputs.length === 0) {
      return null;
    }
    this.numberOfLines = toolTipTextRenderers.length;
    if (this.textInputs.length !== toolTipTextRenderers.length) {
      this.textInputs = Array.apply(null, Array(this.numberOfLines)).map(() =>
